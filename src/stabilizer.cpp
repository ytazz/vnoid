...
void Stabilizer::Step(Robot& robot, Centroid& centroid, Base& base, Foot& foot[2]){
	// copy reference values from iksolver
	centroid.force_ref[0] = robot.total_mass* centroid.com_acc_ref[0];
	centroid.force_ref[1] = robot.total_mass* centroid.com_acc_ref[1];
	centroid.force_ref[2] = robot.total_mass*(centroid.com_acc_ref[2] + robot.gravity);
